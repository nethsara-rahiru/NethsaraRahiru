...
import { 
  ...
  addMaterial, getMaterials, addExam, deleteExam, deleteResult, unpublishResult,
...
const initAdmin = () => {
    ...
    onSnapshot(query(collection(db, "exams"), orderBy("date", "desc")), snapshot => {
        ...
        snapshot.forEach(doc => {
            ...
            const d = document.createElement('div'); d.className='card'; d.style.padding='1.2rem'; d.style.background='rgba(255,255,255,0.02)';
        ...
        document.querySelectorAll('.delete-exam-btn').forEach(btn => btn.onclick = async () => { if(confirm('Delete this exam log?')) await deleteExam(btn.getAttribute('data-id')); });
        refreshIcons();
    });
    ...
    function renderGroupedRegistry(snapshot) {
        ...
        const all = []; snapshot.forEach(doc => {
            ...
            // Backfill names/batches case-insensitively
        ...
        all.forEach(r => {
            ...
            if (!groups[gid]) groups[gid] = { paper: r.examName, part: r.paperPart, sec: r.subSection, batch: r.batch || 'Unassigned', items: [] };
        ...
        Object.values(groups).sort((a,b) => a.paper.localeCompare(b.paper)).forEach((g, idx) => {
            ...
            parentRow.innerHTML = `
                ...
                <td style="padding:1rem;"><span class="mini-tag">${g.batch}</span></td>
            ...
            const individualRows = g.items.map(i => `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:1rem; border-bottom:1px solid rgba(255,255,255,0.03);">
                    ...
                    <div style="display:flex; align-items:center; gap:15px;">
                        <span style="font-weight:800; font-size:1.1rem; color:white;">${i.marks.toFixed(1)}</span>
                        <button class="del-res-btn" data-id="${i.id}" style="padding:4px 8px; font-size:9px; background:rgba(239,68,68,0.1); color:var(--danger);">DEL</button>
                    </div>
                </div>
            `).join('');

            childRow.innerHTML = `<td colspan="4" style="padding: 0 0 2rem 0; background:rgba(0,0,0,0.2);"><div style="padding:1rem;">${individualRows}</div></td>`;
            
            parentRow.onclick = (e) => { if(e.target.tagName !== 'BUTTON') childRow.style.display = childRow.style.display === 'none' ? 'table-row' : 'none'; };
...
async function executeReveal(student) {
    ...
    // Normalize marks to 100% if the part was out of 50
    const rawMarks = latestEntry.marks;
    const displayMarks = (latestEntry.subSection !== 'None') ? rawMarks * 2 : rawMarks;
    
    // Dynamic Local Grade for this specific part
    const getLocalGrade = (m) => { if(m>=75) return 'A'; if(m>=65) return 'B'; if(m>=55) return 'C'; if(m>=40) return 'S'; return 'W'; };
    const localGrade = getLocalGrade(displayMarks);
    ...
    setTimeout(() => {
        ...
        // Use the PART-SPECIFIC grade
        document.getElementById('rev-grade').innerText = localGrade;
        ...
        const tick = () => { 
            if (v < displayMarks) { 
                v += Math.ceil(displayMarks / 40); 
                if (v > displayMarks) v = displayMarks; 
                c.innerText = Math.round(v) + "%"; 
                requestAnimationFrame(tick); 
            } else { 
                c.innerText = Math.round(displayMarks) + "%"; 
            } 
        }; 
        tick();
...
function renderResults(allReleased) {
    ...
    Object.values(groups).forEach(data => {
        const primary = data.final || data.parts[0]; const displayMarks = primary.finalAggregate || primary.marks; const card = document.createElement('div'); card.className='card';
        let segmentsHTML = '';
        data.parts.forEach(p => {
            ...
            segmentsHTML += `<div class="segment ${cls}" style="width: ${p.marks * 0.5}%" data-tooltip="${p.paperPart} ${p.subSection}: ${p.marks.toFixed(1)}"></div>`;
        });
        card.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;"><div><h3 style="font-size:1.15rem; margin:0; font-family:'Outfit';">${primary.examName}</h3><div class="tag-bar" style="margin-top:8px;"><span class="mini-tag">RANK #${primary.rank}</span><span class="mini-tag">GRADE ${primary.grade}</span></div></div><div style="text-align:right;"><span class="marks-counter" style="font-size:2.8rem;">${displayMarks.toFixed(1)}</span></div></div><div class="comparison-line">${segmentsHTML}<div class="comparison-mark" style="left: ${primary.classAverage}%"></div></div><div style="display:flex; justify-content:space-between; font-size:0.75rem; color:var(--text-muted); font-weight:800; letter-spacing:0.5px;"><span>AGGREGATE OUTCOME</span><span>MEAN: ${primary.classAverage}%</span></div><div class="legend-group"><div class="legend-item"><div class="legend-color" style="background:#6ad7ff;"></div> PAPER 1 (A)</div><div class="legend-item"><div class="legend-color" style="background:#7820d0;"></div> PAPER 1 (B)</div><div class="legend-item"><div class="legend-color" style="background:#f59e0b;"></div> PAPER 2 (A)</div><div class="legend-item"><div class="legend-color" style="background:#10b981;"></div> PAPER 2 (B)</div></div>`;
